...
export default function RedeemCheckout() {
	...
	const { deductPoints, setOrders } = useContext(UserContext);
	...
	const handleSubmit = (e) => {
		...
		// Create and save the order to context
		const newOrder = {
			id: Date.now(),
			item,
			formData,
	...
	return (
		<div className="min-h-screen bg-black text-white flex items-center justify-center px-4">
			<div className="w-full max-w-xl bg-neutral-900 p-8 rounded-2xl border border-white/10 shadow-xl">
				<h1 className="text-3xl font-bold text-red-600 mb-6">Redeem Item</h1>

				{/* Item Info */}
				<div className="mb-6 border border-white/10 rounded-lg p-4">
					<h2 className="text-lg font-semibold">{item.name}</h2>
					<p className="text-yellow-400 font-bold mt-2">{item.points} PTS</p>
				</div>
